...
<form method="POST" action="index.php?d=hellovotes&c=votes&a=index"> 
...
foreach($row as $k=>$v)
{
	...
	if($b!=$v['tId'])
		echo "<br/><hr/><a href='index.php?d=hellovotes&c=votes&a=index&p=".$v['tId']."'>".$v['tName']."</a>(".$v['tCountTotal'].")";	
	
	echo "<table class='tab_answer'>";
	if(!empty($v['aName']))
		if($v['tType']=='1'){
			echo "<tr><td>".$v['aName']."</td><td class='spacing
'></td><td class='count'>".$v['aCount']."</td></tr>";
		}
		elseif($v['tType']=='N'){
			echo "<tr><td>".$v['aName']."</td><td class='spacing
'></td><td class='count'>".$v['aCount']."</td></tr>";				
...
<?php 
foreach($que_result as $k=>$v){
	echo "<h4><a href='index.php?d=hellovotes&c=votes&a=result&p=".$v['qId']."'>".$v['qName']."</a></h4>";
}
?>
